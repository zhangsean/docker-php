...
Official `php:apache` with `mod_rewrite` enabled and PHP `gd`, `mysqli`, `pdo_mysql` ext installed.
...
Official `php:7-apache` with `mod_rewrite` enabled and PHP `gd`, `mysqli`, `pdo_mysql` ext installed.
